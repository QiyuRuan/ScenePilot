...
#         self.policy.save_model(model_path)
    ...
    def load_model(self, episode=None, replay_buffer=None):
        if episode is None:
            ...
            load_iteration = self.config.get('load_iteration')
            if load_iteration is not None:
                episode = int(load_iteration)
            else:
                episode = -1
                for _, _, files in os.walk(self.model_path):
                    for name in files:
        ...
        elif self.pretrain_dir != None:
            self.logger.log(f'>> {self.policy_name} pretrain model found at {self.pretrain_dir}', 'green')
        else:
