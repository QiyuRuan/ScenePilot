...
class CarlaBehaviorAgent(BasePolicy):
    ...
    def get_action(self, obs, infos, deterministic=False):
        ...
        for e_i in infos:
            ...
            # CARLA BehaviorAgent expects route state to be refreshed before run_step.
            controller._update_information()
            if controller.done():
                dest_waypoint = e_i['route_waypoints'][-1]
                controller.set_destination(dest_waypoint.transform.location)
                controller._update_information()
