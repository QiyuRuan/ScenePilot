...
class AutoPilot(autonomous_agent.AutonomousAgent):
    ...
    @staticmethod
    def _slice_adv_actions_for_batch(adv_actions, batch_id):
        if adv_actions is None:
            return None

        sliced = {}
        for action_name, action_value in adv_actions.items():
            if action_value is None:
                sliced[action_name] = None
            elif hasattr(action_value, "dim") and action_value.dim() >= 3:
                sliced[action_name] = action_value[batch_id]
            else:
                sliced[action_name] = action_value
        return sliced
    ...
    def _get_control(self, input_data, adv_actions=None, steer_init=None, throttle_init=None,
        ...
        for ix in range(len(input_data['gps'])):
            ...
            current_adv_actions = self._slice_adv_actions_for_batch(adv_actions, ix)
            if vehicle_hazard_init is None or light_hazard_init is None or walker_hazard_init is None or stop_sign_hazard_init is None:
                brake = self._get_brake(ix, vehicle_hazard_init, light_hazard_init, walker_hazard_init, stop_sign_hazard_init, adv_actions=current_adv_actions) # privileged
            else:
        ...
        if vehicle_hazard is None or walker_hazard is None:
            ...
            for ix in range(adv_vehicles_state["pos"][batch_id].shape[0]):
                ...
                if (adv_loc.distance(vehicle_location) < detection_radius):
                    ...
                    traffic_control.steer = adv_actions["steer"][ix].detach().cpu().item()
                    traffic_control.throttle = adv_actions["throttle"][ix].detach().cpu().item()
                    traffic_control.brake = adv_actions["brake"][ix].detach().cpu().item()
