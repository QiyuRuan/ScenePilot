...
class GenerationEngine:
    ...
    def unroll_simulation(self):
        ...
        for t in range(self.args.sim_horizon):
            ...
            if not hasattr(self, "_ego_policy_accepts_adv_actions"):
                params = inspect.signature(self.simulator.ego_policy.run_step).parameters
                self._ego_policy_accepts_adv_actions = "adv_actions" in params

            if self._ego_policy_accepts_adv_actions:
    ...
    def compute_cost(self):
        ...
        if adv_col_cost.size(-1) == 0:
            adv_col_cost = ego_col_cost.new_zeros((1, 1))
            assert adv_col_cost.size(0) == 1, 'This works only for batchsize 1!'

        adv_col_cost = torch.minimum(
            adv_col_cost,
            adv_col_cost.new_tensor([self.args.adv_col_thresh]).float(),
        )
