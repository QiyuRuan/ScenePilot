...
def _build_global_route_planner(world_map, hop_resolution):
    if GlobalRoutePlannerDAO is not None:
        ...
        if hasattr(grp, "setup"):
            grp.setup()
        return grp
    return GlobalRoutePlanner(world_map, hop_resolution)
...
def interpolate_trajectory(world, waypoints_trajectory, hop_resolution=1.0):
    ...
    grp = _build_global_route_planner(world.get_map(), hop_resolution)
    # Obtain route plan
    route = []
