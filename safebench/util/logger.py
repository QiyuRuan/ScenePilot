...
class Logger:
    ...
    def init_wandb(self, use_wandb=False, project=None, entity=None, name=None, group=None, mode='online', config=None):
        if not use_wandb:
            return
        if wandb is None:
            self.log(">> wandb is not installed; skipping Weights & Biases logging.", 'yellow')
            return
        if self.wandb_run is not None:
            return

        run_name = name or self.exp_name
        self.wandb_run = wandb.init(
            project=project or 'SafeBench',
            entity=entity or None,
            name=run_name,
            group=group or None,
            mode=mode,
            dir=self.output_dir,
            config=convert_json(config or {}),
            reinit=True,
        )
        self.log(f'>> wandb initialized: project={self.wandb_run.project}, name={self.wandb_run.name}', 'green')
