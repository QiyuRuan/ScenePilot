...
from safebench.scenario.tools.king_generation import maybe_prepare_king_scenarios
...
def scenario_parse(config, logger):
    ...
    maybe_prepare_king_scenarios(config, logger)
    logger.log(">> Parsing scenario route and data")
    if osp.isabs(config['scenario_type']):
        list_of_scenario_config = config['scenario_type']
    else:
    ...
    for item in data_full:
        ...
        parsed_config.bv_num = config.get('bv_num', 0)
