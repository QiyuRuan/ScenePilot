...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        ...
        self.walker_base_speed = 1.4     # m/s, normal walking
        self.walker_speed_scale = 0.8    # Speed range contributed by the action
        self.walker_max_speed = 3.0      # Upper limit to avoid unrealistic running
        self.walker_max_yaw_offset = 30  # Maximum yaw offset in degrees (+/-30 deg)
    ...
    def convert_actions(self, actions):
        """
        actions: [a_speed, a_dir] ∈ [-1, 1]^2
          - a_speed controls pedestrian speed
          - a_dir controls yaw offset relative to the current heading.
        Returns:
          speed: scalar
          direction: normalized Vector3D direction
        """
        a_speed = float(actions[0])
        a_dir   = float(actions[1])

        # 1) Speed: base +/- scale
        speed = self.walker_base_speed + self.walker_speed_scale * a_speed
        speed = max(0.0, min(self.walker_max_speed, speed))

        # 2) Direction: add a yaw offset to the current forward_vector yaw.
        actor_tf = CarlaDataProvider.get_transform(self.other_actors[0])
        base_yaw_deg = actor_tf.rotation.yaw              # Already points in the crossing direction.
        delta_yaw_deg = a_dir * self.walker_max_yaw_offset  # [-max, max]
        new_yaw_rad = math.radians(base_yaw_deg + delta_yaw_deg)

        # Build the new direction vector by rotating only in the XY plane.
        direction = carla.Vector3D(
            math.cos(new_yaw_rad),
            math.sin(new_yaw_rad),
            0.0
        )
    ...
    def update_behavior(self, scenario_action):
        speed, direction = self.convert_actions(scenario_action)

        # Apply WalkerControl directly to the walker.
        control = self.other_actors[0].get_control()
        control.speed = speed
        control.direction = direction
        # Leave fields like is_jump unchanged for now.
