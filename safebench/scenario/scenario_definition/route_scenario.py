...
class RouteScenario():
    ...
    def initialize_actors(self):
        amount = getattr(self.config, 'bv_num', 0)
    ...
    def get_running_status(self, running_record):
        ...
        for scenario in self.list_scenarios:
            for actor in scenario.other_actors:
                ...
                actor_acc = actor.get_acceleration()
                adv_actor_status[str(actor.id)] = {
                    'x': actor_trans.location.x,
                    'y': actor_trans.location.y,
                    'z': actor_trans.location.z,
                    'roll': actor_trans.rotation.roll,
                    'pitch': actor_trans.rotation.pitch,
                    'yaw': actor_trans.rotation.yaw,
                    'velocity_x': actor_vel.x,
                    'velocity_y': actor_vel.y,
                    'velocity_z': actor_vel.z,
                    'acceleration_x': actor_acc.x,
                    'acceleration_y': actor_acc.y,
                    'acceleration_z': actor_acc.z,
                    'role_name': actor.attributes.get('role_name', '')
                }
        running_status['adv_actors'] = adv_actor_status
    ...
    def clean_up(self):
        ...
        for s_i in range(len(self.background_actors)):
            if self.background_actors[s_i].type_id.startswith('vehicle'):
                self.background_actors[s_i].set_autopilot(
                    False,
