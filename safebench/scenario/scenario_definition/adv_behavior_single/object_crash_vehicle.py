...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        ...
        self._fixed_walk_direction = None

    def _is_route13_trigger(self):
        # Route 13 trigger in scenario_01.json: (-93.87, 144.33)
        target_x = -93.87
        target_y = 144.33
        tol = 0.4
        return (
            abs(self._trigger_location.x - target_x) <= tol and
            abs(self._trigger_location.y - target_y) <= tol
        )
    ...
    def initialize_actors(self):
        ...
        if self._is_route13_trigger():
            forward = self.transform.rotation.get_forward_vector()
            self._fixed_walk_direction = carla.Vector3D(-forward.x, -forward.y, -forward.z)
    ...
    def update_behavior(self, scenario_action):
        ...
        if self._fixed_walk_direction is not None:
            self.scenario_operation.walker_go_straight(other_actor_speed, 0, direction=self._fixed_walk_direction)
        else:
