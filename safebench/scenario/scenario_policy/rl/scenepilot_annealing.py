...
class PolicyNet(nn.Module):
    ...
    def forward(self, x):
        ...
        std = (self.softplus(self.fc_std(h)) + self.min_std).clamp(max=0.5)
        return mu, std
...
class ScenePilot(BasePolicy):
    ...
    def __init__(self, config, logger):
        ...
        self.clip_eps   = float(cm.get('ppo_clip', 0.2))
        self.feas_tol   = float(cm.get('feas_tol', 0.0))

        # Threshold mode: off, fixed, or Gaussian.
        self.k_mode   = cm.get('k_mode', 'off')           # 'off' -> relative threshold
        self.beta     = float(cm.get('beta_init', 0.90))
        self.beta_step= float(cm.get('beta_step', 0.01))
        self.beta_max = float(cm.get('beta_max', 0.99))
        self.k        = float(cm.get('k_start', 0.01))
        self._k_pending = None  # New k value to apply on the next episode.
        self.k_step   = float(cm.get('k_step', 0.01))
        self.k_max    = float(cm.get('k_max', 0.40))
        ...
        self.gamma     = float(cm.get('gamma', 0.99))
        self.lam       = float(cm.get('gae_lambda', 0.95)) 
        ...
        self.mode = 'train'
        self.prev_J_sigma = None

        self.gated = bool(cm.get('gated', False))

        self.thresh_update_every = int(config.get('scenepilot', {}).get('thresh_update_every', 50))
        self._thresh_tick = 0


        self.ent_start = 0.18
        self.ent_end   = 0.02
        self.ent_decay_steps = 1000   # Linear annealing steps counted by train() calls.
        self._update_step = 0
    ...
    def _ent_coef_now(self):
        t = min(self._update_step, self.ent_decay_steps)
        frac = t / max(1, self.ent_decay_steps)
    ...
    def train(self, replay_buffer, ep_stats=None):
        ...
        if self._k_pending is not None:
            ...
            if not math.isclose(new_k, old_k, rel_tol=0.0, abs_tol=1e-12):
                if self.logger:
                    self.logger.log(f"[ScenePilot] apply deferred k switch: {old_k:.4f} -> {new_k:.4f}", "cyan")
                self.k = new_k
                ...
                if os.path.isdir(k_dir_path):
                    try:
                        for n in os.listdir(k_dir_path):
                            if n.startswith('model.scenepilot.') and n.endswith('.torch'):
                                has_ckpt = True
                                break
                    except Exception:
                        pass
        ...
        # 1) Record episode-level statistics for thresholding.
        if ep_stats:
            ...
            if J_sigma is not None:
                if self.prev_J_sigma is None:
                    self.prev_J_sigma = J_sigma
                if self.logger:
        ...
        for _ in range(self.epochs):
            ...
            for start in range(0, B, batch_size):
                ...
                ent_coef_now = self._ent_coef_now()
                policy_loss = -(torch.min(surr1, surr2).mean() + ent_coef_now * entropy.mean())
    ...
    def _sigma_threshold_tensor(self):
        if self.k_mode == 'gaussian':
            base = self.prev_J_sigma if self.prev_J_sigma is not None else 0.0
            th = self.beta * base
        else:
            th = self.k
        return torch.tensor(th, dtype=torch.float32)


    def _advance_threshold(self, feasible: bool):

        # Gaussian threshold-frequency mode: scan lower thresholds more often and higher thresholds less often.
        if self.k_mode == 'gaussian':
            return
        
        # Absolute threshold mode (epsilon = k): update k by window.
        self._thresh_tick += 1
        ...
        # Avoid 0.499999999-style floating-point comparisons against k.max.
        if (self.k_max - self.k) <= 1e-8:
            new_k_val = 0.0
        else:
            new_k_val = min(self.k_max, self.k + self.k_step)
        
        self._thresh_tick = 0

        if abs(new_k_val - self.k) > 1e-12:
    ...
    def load_model(self, arg=None):
        ...
        if isinstance(arg, list):
            ...
            for cfg in scenario_configs:
                ...
                model_path = os.path.join(self.model_path, str(self.scenario_id), str(self.route_id), model_file)
                if os.path.isfile(model_path):
                    self.logger.log(f'>> Loading {self.name} model from {model_path}')
                    ckpt = torch.load(model_path, map_location='cpu')
                    self.policy.load_state_dict(ckpt['policy'])
                else:
                    self.logger.log(f'>> Fail to find {self.name} model at {model_path}', color="yellow")
            return
