...
class PPO(BasePolicy):
    ...
    def info_process(self, infos):
        ...
        return self._prepare_state(info_batch)
    ...
    def train(self, replay_buffer, ep_stats=None):
        ...
        for K in range(self.train_iteration):
            ...
            bn_s = CUDA(torch.FloatTensor(self._prepare_state(batch['actor_info'])))
            bn_s_ = CUDA(torch.FloatTensor(self._prepare_state(batch['n_actor_info'])))
            bn_a = CUDA(torch.FloatTensor(batch['action']))
            reward = CUDA(torch.FloatTensor(batch['reward']))
            if reward.ndim == 1:
                bn_r = reward.unsqueeze(-1)
            else:
                # In AV-safe training, env provides [adv_reward, sigma].
                bn_r = reward[:, :1] + torch.clamp(reward[:, 1:2], max=0.0)
    ...
    def save_model(self, episode):
        ...
        filepath = os.path.join(save_dir, f'model.ppo.{self.model_id}.{episode:04}.torch')
