...
class SAC(BasePolicy):
    ...
    def get_action(self, state, infos, deterministic=False):
        ...
        # actor use softplus output, so this lead to nan because exp.
    ...
    def get_action_log_prob(self, state):
        ...
        log_prob = dist.log_prob(z) - torch.log(1 - action.pow(2) + self.min_Val)
        log_prob = torch.sum(log_prob, dim=1, keepdim=True)
        return action, log_prob, z, mu, torch.log(std + 1e-8)
    ...
    def save_model(self, episode):
        ...
        save_dir = os.path.join(self.model_path, str(self.scenario_id), str(self.route_id))
    ...
    #     else:
    #         self.logger.log(f'>> Fail to find {self.policy_type} model from {save_dir}', color='yellow')
    ...
    def load_model(self, arg=None):
        ...
        1) load_model(scenario_configs: list)  - Evaluation loads by scenario ID
        2) load_model(episode: int|None)      - Training/resume loads the latest checkpoint by episode
        - arg=None is equivalent to episode=None
        """
        ...
        if isinstance(arg, list):
            ...
            for cfg in scenario_configs:
                ...
                model_file = cfg.parameters              # File name specified in YAML
                # model_path = os.path.join(self.model_path, str(scenario_id),str(route_id), model_file )
                # trainav only the path is modefied in run.py
                model_path = os.path.join(self.model_path, model_file )
                if os.path.isfile(model_path):
                    self.logger.log(f'>> Loading {self.policy_type} model from {model_path}')
                    ckpt = torch.load(model_path, map_location='cpu')
                    self.policy_net.load_state_dict(ckpt['policy_net'])
                    self.value_net.load_state_dict(ckpt['value_net'])
                    self.Q_net.load_state_dict(ckpt['Q_net'])
                else:
                    self.logger.log(f'>> Fail to find {self.policy_type} model at {model_path}', color="yellow")
            return  # Return after scenario-based loading.

        # Load by episode
        episode = arg  # None or int
        if episode is None:
        ...
        # save_dir = os.path.join(self.model_path, str(self.scenario_id), str(self.route_id))
        # trainav only the path is modefied in run.py
        save_dir = os.path.join(self.model_path)
        ckpt_file = os.path.join(save_dir, f'model.sac.{self.model_id}.{episode:04}.torch')
        if os.path.isfile(ckpt_file):
            self.logger.log(f">> Loading scenario policy {self.name} model from {ckpt_file}")
            ...
            self.logger.log(f'>> Fail to find {self.policy_type} model at {ckpt_file}', color="yellow")
