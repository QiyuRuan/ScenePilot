...
class RoutePlanner():
    def __init__(self, vehicle, buffer_size, init_waypoints):
        ...
        self._target_waypoint = None # av target
        self._npc_goal_waypoint = None #npc target
    ...
    def run_step(self):
        waypoints, target_road_option, current_waypoint, target_waypoint, npc_goal_waypoint = self._get_waypoints()
        red_light, vehicle_front = self._get_hazard()
        # red_light = False
        return waypoints, target_road_option, current_waypoint, target_waypoint, npc_goal_waypoint, red_light, vehicle_front
    ...
    def _get_waypoints(self):
        ...
        # npc_goal_waypoint for npc local target
        if len(self._waypoint_buffer) > 0:
            idx = len(self._waypoint_buffer) // 3  # from FREA: neither far nor close yet
            self._npc_goal_waypoint, _ = self._waypoint_buffer[idx]
        else:
            self._npc_goal_waypoint = self._current_waypoint
        ...
        return waypoints, self._target_road_option, self._current_waypoint, self._target_waypoint, self._npc_goal_waypoint
