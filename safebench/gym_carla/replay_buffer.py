...
class RouteReplayBuffer:
    ...
    def __init__(self, num_scenario, mode, buffer_capacity=1000, mix_collision=False ):
        self.mode = mode
        ...
        # Controls whether collision trajectories are mixed in; enable this for ChatScene-style AV-safe training.
        self.mix_collision = mix_collision
    ...
    def reset_buffer(self):
        ...
        self.buffer_next_obs = [[] for _ in range(self.num_scenario)]
         # Unified reward buffer for agent or scenario rewards.
    ...
    # Original scenario-training path
    ...
    #         # ---------- Store reward ----------
    #         # Use the fused reward from info when training the scenario policy.
    #         # if 'npc_reward' in additional_dict[s_i]:          # train_scenario
    #         if self.mode == 'train_scenario':          # train_scenario
    ...
    #             if key == 'scenario_id':
    #                 continue
    #             if key == 'npc_reward':                       # Already stored in the main reward buffer.
    #                 continue
    ...
    def store(self, data_list, additional_dict):
        ...
        # ChatScene AV-safe training path
        if self.mix_collision:
            # separate trajectories according to infos
            for s_i in range(len(additional_dict)):
                sid = additional_dict[s_i]['scenario_id']
        ...
        # Original scenario-training path
            ...
            for s_i in range(len(additional_dict)):
                ...
                # if 'npc_reward' in additional_dict[s_i]:          # train_scenario
                if self.mode == 'train_scenario':          # train_scenario
                    # sigle objective need to run this
                    # self.buffer_rewards[sid].append(additional_dict[s_i]['npc_reward'])

                    # muti objective need to run this
                    self.buffer_rewards[sid].append(additional_dict[s_i]['npc_moreward_vec'])
                else:                                         # train_agent
                    self.buffer_rewards[sid].append(rewards[s_i])
                ...
                for key in additional_dict[s_i].keys():
                    ...
                    if key == 'npc_reward':                       # Already stored in the main reward buffer.
                        continue
                    if key == 'npc_moreward_vec':                       # Already stored in the main reward buffer.
                        continue
                    if key not in self.buffer_additional_dict[s_i].keys():
                        self.buffer_additional_dict[s_i][key] = []
                    self.buffer_additional_dict[s_i][key].append(additional_dict[s_i][key])
    ...
    def sample(self, batch_size):
        ...
        # Choose between ChatScene AV-safe training data and regular scenario-training data.
        if self.mode == 'train_agent' and self.mix_collision:
        ...
        # Original scenario-training path
            ...
            for k_i in prepared_infos.keys():
                if k_i == 'route_waypoints':
                    continue
                if k_i in ['sigma_t', 'sigma_tp1']:
                    # 
                    batch[k_i] = np.stack(prepared_infos[k_i])[sample_index]
                else:
                    # Original t / t+1 logic
                    batch_info[k_i] = np.stack(prepared_infos[k_i])[sample_index-1]
                    batch_info['n_' + k_i] = np.stack(prepared_infos[k_i])[sample_index]
    ...
    def sample_scenepilot_rollout(self):
        """
        Sampling helper used by ScenePilot:
        - Keeps temporal order instead of random sampling
        - Outputs state, n_state, action, reward, done,
            plus actor_info / n_actor_info, sigma_t / sigma_tp1, and related fields
        """
        # 1) Reuse the preparation block from sample(), excluding the collision branch.
        ...
        samples_per_trajectory = self.buffer_capacity // self.num_scenario
        ...
        for s_i in range(self.num_scenario):
            ...
            for k_i in self.buffer_additional_dict[s_i].keys():
                if k_i not in prepared_infos:
                    prepared_infos[k_i] = []
                prepared_infos[k_i] += self.buffer_additional_dict[s_i][k_i][start_idx:]

        total_steps = len(prepared_rewards)
        # At least two steps are required to build (t, t+1).
        if total_steps <= 1:
            raise ValueError(f"[sample_scenepilot_rollout] not enough steps: {total_steps}")

        # 2) Build temporally ordered indices: 1..T-1.
        idx = np.arange(1, total_steps)

        # ScenePilot is a scenario policy, so use scenario_actions.
        action = prepared_scenario_actions

        batch = {
            'action': np.stack(action)[idx],                # [T, act_dim]
            'state':  np.stack(prepared_obs)[idx],          # s_t
            'n_state':np.stack(prepared_next_obs)[idx],     # s_{t+1}
            'reward': np.stack(prepared_rewards)[idx],      # r_t
            'done':   np.stack(prepared_dones)[idx],        # done_t
        }

        # 3) Process additional info fields.
        batch_info = {}
        for k_i in prepared_infos.keys():
            if k_i == 'route_waypoints':
                continue

            arr = np.stack(prepared_infos[k_i])   # [total_steps, ...]
            if k_i in ['sigma_t', 'sigma_tp1']:
                # sigma_t / sigma_tp1 is already frame-wise, so (t, t+1) is encoded in the values.
                batch[k_i] = arr[idx]
            else:
                # Treat other info fields as state attributes and create t and t+1 versions.
                batch_info[k_i]        = arr[idx - 1]   # Corresponds to s_t
                batch_info['n_' + k_i] = arr[idx]       # Corresponds to s_{t+1}
