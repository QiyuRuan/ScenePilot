...
class VectorWrapper():
    """ 
        ...
        Wrapper around each lowest-level env instance.
    """
    ...
    def __init__(self, env_params, scenario_config, world, birdeye_render, display, logger):
        ...
        for i in range(self.num_scenario):
            env = carla_env(env_params, birdeye_render=birdeye_render, display=display, world=world, logger=logger) # Pass rl_safe parameters.
    ...
    def step(self, ego_actions, scenario_actions):
        ...
        for e_i in range(self.num_scenario):
            if not self.finished_env[e_i]:
                ...
                # env_list[e_i] is the individual lowest-level carla_env.
...
class ObservationWrapper(gym.Wrapper):
    ...
    def _build_obs_space(self):
        if self.obs_type == 0:
            ...
            obs_lim = np.ones((obs_dim), dtype=np.float32)
            self.observation_space = gym.spaces.Box(-obs_lim, obs_lim)
        elif self.obs_type == 2 or self.obs_type == 3 or self.obs_type == 4:
            # 4 state space + bev
    ...
    def _extract_lidar_points(self):
        if getattr(self._env, 'disable_lidar', True):
            return np.zeros((0, 3), dtype=np.float32)
        lidar_data = getattr(self._env, 'lidar_data', None)
        if lidar_data is None:
            return np.zeros((0, 3), dtype=np.float32)
        try:
            point_cloud = np.copy(np.frombuffer(lidar_data.raw_data, dtype=np.dtype('f4')))
            point_cloud = np.reshape(point_cloud, (int(point_cloud.shape[0] / 4), 4))
            return point_cloud[:, :3].astype(np.float32)
        except Exception:
            return np.zeros((0, 3), dtype=np.float32)
    ...
    def _preprocess_obs(self, obs):
        ...
        # dedicated observation for TransFuser-style sensor fusion
        elif self.obs_type == 4:
            ego_vehicle = getattr(self._env, 'ego_vehicle', None)
            gps = np.zeros(2, dtype=np.float32)
            compass = 0.0
            speed = float(obs['state'][2]) if 'state' in obs else 0.0

            if ego_vehicle is not None:
                transform = ego_vehicle.get_transform()
                gps = np.array([transform.location.x, transform.location.y], dtype=np.float32)
                compass = float(transform.rotation.yaw / 180.0 * np.pi)

            return {
                "camera": obs['camera'],
                "lidar_points": self._extract_lidar_points(),
