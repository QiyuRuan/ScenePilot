...
if __name__ == '__main__':
    ...
    parser.add_argument('--tag', type=str, default="1-13", help='short run tag shown in logs/dirs')
    ...
    parser.add_argument('--train_env_steps', type=int, default=None, help='stop train loop after this many env steps')
    parser.add_argument('--train_env_step_offset', type=int, default=0, help='global env-step offset for logging')
    parser.add_argument('--train_avsafe_steps', type=int, default=None, help='stop train loop after this many av-safe update steps')
    parser.add_argument('--train_avsafe_step_offset', type=int, default=0, help='global av-safe step offset for logging')
    parser.add_argument('--max_wait_collision_episodes', type=int, default=None, help='skip route if no collision appears within this many episodes')
    parser.add_argument('--route_level_avsafe_training', action='store_true', default=False)
    parser.add_argument('--avsafe_training', type=int, choices=[0, 1], default=None, help='override rl_config av_safe.training')
    ...
    if args.output_dir is None:
        args.output_dir = f"log_scenario/log_{run_tag}"
    ...
    log_path = os.path.join("console", "scenario",
                            f'console_{run_tag}.txt')
    ansi_re = re.compile(r"\x1B\[[0-?]*[ -/]*[@-~]")   # Generic match
    os.makedirs("console/scenario", exist_ok=True)
    ...
    for agent_cfg in args.agent_cfg:
        for scenario_cfg in args.scenario_cfg:
            ...
            if args.avsafe_training is not None:
                rl_config['av_safe']['training'] = bool(args.avsafe_training)
